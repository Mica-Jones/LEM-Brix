...
```{r data import}
tno3 <- read_csv("../data/incubation_tin_round1.csv")
...
tno3w <- tno3 %>%
  mutate(rep = factor(rep),
         ...
         day = factor(day),
         lem = factor(lem)) %>%
  mutate(trtname = paste0(location,"_",rate, "_", lem))
...
```{r nk boxplot}
ggplot(tno3w, aes(x = day, 
                    y = tinmgkg,
                    color = rate)) +
  geom_boxplot() +
  geom_jitter() +
  facet_grid(location~lem) # tilde controls what axis the facets are laid out on; krate_kgha~. would transpose; nrate_kgha~krate_kgha would create a different facet for each treatment category
  theme(legend.position = "none")
  
    ggsave("../output/incubationno31.png",
       width = 8,
       height = 5)
```
...
# Model fitting
tno3_mod <- lm(brix ~ rep + lem*location, # rep + is functionally the only coding difference between a CRD and a RCBD design)
               data = tno3w)
tno3_mod
...
```{r rcbd_resid}
tno3_resid <- augment(tno3_mod) %>%
  mutate(.studresid=rstudent(tno3_mod))
...
```{r }
ggplot(tno3_resid, aes(x=.fitted, y=.studresid))+
...
```{r}
ggplot(tno3_resid, aes(sample=.studresid))+
  stat_qq(  shape = 21,
            fill = "purple", 
            size = 3,
            alpha = .7
...
```{r}
ggplot(tno3_resid, aes(x=.studresid))+
...
tno3_means_all <- emmeans(tno3_mod,
                          ~lem:location)
...
tno3_cld_all <- cld(tno3_means_all, 
                   reversed=T, 
                   adjust="none",
               Letters=letters,
               alpha = 0.05)
...
```{r selected pwc}
tno3_cld_selected <- tno3_cld_all %>%
  as.data.frame() %>%
...
geom_jitter(data = tno3w,
            aes(x = trtname,
                y = brix,
                color = trtname),
            alpha = 0.6) +
  geom_label(data = tno3_cld_selected,
            aes(x = trtname,
                y = emmean,
                label = letter),
            size = 3,
...
ggplot(mapping = aes(fill = trtname))+
  ...
  geom_jitter(data = tno3w,
              ...
              size = 3,
              alpha = .6) +
  facet_grid(lem~location)
  labs(x = "Location and LEM Rate",
       y = "Nitrate (mg/kg dry soil") +
  scale_fill_viridis_d() +
