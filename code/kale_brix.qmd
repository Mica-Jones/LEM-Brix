...
```{r data import}
brix_k <- read_csv("../data/brix_kale_2024.csv")
...
brix_kw <- brix_k %>%
  ...
  mutate(trtname = paste0(location,"_",lem))
...
ggplot(brix_kw, aes(x = lem, 
  ...
  geom_boxplot() +
  geom_jitter() +
  facet_grid(.~location) # tilde controls what axis the facets are laid out on; krate_kgha~. would transpose; nrate_kgha~krate_kgha would create a different facet for each treatment category
  theme(legend.position = "none")
```
...
# Model fitting
brixk_mod <- lm(brix ~ rep + lem*location, # rep + is functionally the only coding difference between a CRD and a RCBD design)
               data = brix_kw)
brixk_mod
...
```{r rcbd_resid}
brixk_resid <- augment(brixk_mod) %>%
  mutate(.studresid=rstudent(brixk_mod))
...
```{r }
ggplot(brixk_resid, aes(x=.fitted, y=.studresid))+
...
```{r}
ggplot(brixk_resid, aes(sample=.studresid))+
  stat_qq(  shape = 21,
            fill = "purple", 
            size = 3,
            alpha = .7
...
```{r}
ggplot(brixk_resid, aes(x=.studresid))+
...
brixk_means_all <- emmeans(brixk_mod,
                          ~lem:location)
...
brixk_cld_all <- cld(brixk_means_all, 
                   reversed=T, 
                   adjust="none",
               Letters=letters,
               alpha = 0.05)
...
```{r selected pwc}
brixk_cld_selected <- brixk_cld_all %>%
  as.data.frame() %>%
  mutate(letter = trimws(.group)) %>% 
  mutate(trtname = paste0(location, lem))
...
geom_jitter(data = brix_kw,
            aes(x = trtname,
                y = brix,
                color = trtname),
            alpha = 0.6) +
  geom_label(data = brixk_cld_selected,
            aes(x = trtname,
                y = emmean,
                label = letter),
            size = 3,
            label.padding = unit(.1, "lines")) +
  labs(x = "Location and LEM Rate",
       y = "%Brix") +
...
ggplot(mapping = aes(fill = trtname))+
  ...
  # Adding letters
  geom_label(data = brixk_cld_selected,
            aes(x = lem, y = emmean, label = letter),
            fill = "white") +
  facet_grid(.~location)
  labs(title = "2024 Kale Brix by LEM Treatment", 
       x = "LEM Rate") +
  scale_fill_viridis_d() +
...
ggsave("../output/brix_kale_shitty.png",
       width = 8,
       height = 5)
```
