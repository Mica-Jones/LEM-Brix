...
```{r data import}
brix_c <- read_csv("../data/brix_carrot_2024.csv")
...
# Model fitting
brixc_mod <- lm(brix ~ rep + lem*location, # rep + is functionally the only coding difference between a CRD and a RCBD design)
               data = brix_cw)
brixc_mod
...
```{r rcbd_resid}
brixc_resid <- augment(brixc_mod) %>%
  mutate(.studresid=rstudent(brixc_mod))
...
```{r }
ggplot(brixc_resid, aes(x=.fitted, y=.studresid))+
...
```{r}
ggplot(brixc_resid, aes(sample=.studresid))+
  stat_qq(  shape = 21,
            fill = "purple", 
            size = 3,
            alpha = .7
...
```{r}
ggplot(brixc_resid, aes(x=.studresid))+
...
brixc_means_all <- emmeans(brixc_mod,
                          ~lem:location)
...
brixc_cld_all <- cld(brixc_means_all, 
                   reversed=T, 
                   adjust="none",
               Letters=letters,
               alpha = 0.05)
...
```{r selected pwc}
brixc_cld_selected <- brixc_cld_all %>%
  as.data.frame() %>%
...
geom_jitter(data = brix_cw,
            aes(x = trtname,
                y = brix,
                color = trtname),
            alpha = 0.6) +
  geom_label(data = brixc_cld_selected,
            aes(x = trtname,
                y = emmean,
                label = letter),
            size = 3,
...
ggsave("../output/brix_carrots_shitty.png",
       width = 8,
       height = 5)
```
