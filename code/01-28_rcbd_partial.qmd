...
ggplot(rcbd_dfw, aes(x = nrate_kgha, 
  ...
  geom_boxplot() +
  geom_jitter() +
  facet_grid(.~krate_kgha) # tilde controls what axis the facets are laid out on; krate_kgha~. would transpose; nrate_kgha~krate_kgha would create a different facet for each treatment category
  theme(legend.position = "none")
```
...
# Model fitting
rcbd_mod <- lm(yield_kgha ~ rep + nrate_kgha*krate_kgha, # rep + is functionally the only coding difference between a CRD and a RCBD design)
               data = rcbd_dfw)
rcbd_mod
